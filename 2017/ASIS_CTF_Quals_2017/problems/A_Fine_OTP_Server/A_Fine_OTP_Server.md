...
```
The template phrase is `60` bytes long, and OTP\_1, which includes
the passphrase, is `78` bytes, or `624` bits long. $$ OTP_{1}^{3} $$ is on the order of 624*3 bits long. $$ 624*3=1872<2048 bits $$ , so this doesn't wrap around the modulus! We just need to take the cubed root of one of the given messages, and provide that to the server to get the flag.
